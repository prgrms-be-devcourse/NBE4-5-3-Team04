...
export default function PostContent({
  ...
  const handleFollowToggle = async (userId: number) => {
    ...
    try {
      ...
      await client.POST("/api/posts/{postId}/follow", {
        // 엔드포인트 수정 및 postId 사용
        body: requestBody,
        params: {
          // params 속성 추가
    ...
    <div>
      ...
      <div className="flex items-center mt-4 space-x-3">
        ...
        <Button
          variant="outline"
          onClick={() => {
            if (post.authorDTO?.memberId) {
              handleFollowToggle(post.authorDTO.memberId);
            }
          }}
        >
          팔로우
            ...
            {post.imageUrls!.map((_, index) => (
              <span
                ...
                className={`h-2 w-2 rounded-full ${
                  index === (activeIndices || 0) ? "bg-blue-500" : "bg-gray-400"
                }`}
              ></span>
            ))}
          </div>

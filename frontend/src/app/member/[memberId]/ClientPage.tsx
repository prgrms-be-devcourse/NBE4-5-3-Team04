...
import { Button } from "@/components/ui/button"; // Next.js Link 컴포넌트 사용
    ...
    <div className="flex flex-col items-center w-full bg-white p-6">
      <div className="flex flex-col items-center w-full bg-white p-6">
        ...
        <ProfileImageEditor
          ...
          memberId={memberId}
          isEditable={profileData.isMe ?? false}
        ...
        <div className="flex justify-center gap-8 mt-6">
          ...
          <div className="text-center">
            <Link href={`/member/follow/${memberId}?tab=follower`}>
              {/* 팔로워 링크 */}
              <p className="text-lg font-bold">
                {profileData.totalFlowerCount}
              </p>
              <p className="text-sm text-gray-600">팔로워</p>
            </Link>
          </div>
          <div className="text-center">
            <Link href={`/member/follow/${memberId}?tab=following`}>
              {/* 팔로잉 링크 */}
              <p className="text-lg font-bold">
                {profileData.totalFlowingCount}
              </p>
